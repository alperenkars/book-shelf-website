...
const followSql = fs.readFileSync(path.join(__dirname, 'book_copy.sql'), 'utf-8');
...
connection.query(followSql, (err, results) => {
  if (err) {
    console.error('Error executing book_copy.sql:', err.message);
    connection.end();
    return;
  }
  console.log('BookCopy data inserted successfully.');
